...
const cpuPctFallback = 0;
const memPctFallback = 0;
...
const ResourceOverview = () => {
  ...
  const topMemory = data?.data.topMemory ?? [];
  const cpuPct = data?.data.cpuPct ?? cpuPctFallback;
  const memPct = data?.data.memPct ?? memPctFallback;
  const maxMem = Math.max(...topMemory.map((t) => t.usageGB), 1);
  return (
          ...
          {topMemory.map((t) => {
            ...
            return (
              <div key={t.name} className="flex items-center gap-3">
                ...
                <span className="text-xs font-semibold text-foreground w-12 text-right">{t.usageGB} GB</span>
              </div>
            );
          })}
        </div>
